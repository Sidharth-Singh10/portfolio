...
tags: [Rust]
draft: false
summary: Traits with a simpler examples
---

Understanding the Rust book defintions can be a tedious task espicially when you are new to programming and aren't yet familiar with the techincal jargons.

Let's just get started with the formal definition mentioned in the rust book.<br/>
...
This means a `Trait` is simply a way to define **common behavior** that muiltple types can implement in their own way (similar to  `Interface` if you are coming from other languages)
