...
export const getServerSideProps = async () => {
  const response = await fetch(
    'https://api.openweathermap.org/data/2.5/weather?lat=26.8467&lon=81.0255&appid=1b3c10c18e894eaf1fd63eedde53fa54&units=metric'
  )
...
export default function Now(currentlyReading) {
  ...
  var SidBirthDate = '2003-03-01'
  var birthDate = new Date(SidBirthDate)

  var SidAge = year - birthDate.getFullYear()

  var SidMonth = 0
  if (month >= birthDate.getMonth()) SidMonth = month - birthDate.getMonth()
  else {
    SidAge--
    SidMonth = 12 + month - birthDate.getMonth()
  }

  var SidDay = 0
  if (date >= birthDate.getDate()) SidDay = date - birthDate.getDate()
  else {
    SidMonth--
    SidDay = 31 + date - birthDate.getDate()
    if (SidMonth < 0) {
      SidMonth = 11
      SidAge--
    }
        ...
        <div>
          <div className="flex justify-between gap-5">
            <div className="mt-2 mb-10 w-1/2 rounded-md border border-gray-600 p-1 text-sm dark:border-gray-200">
              <span className="ml-2 font-semibold">Location:</span> <span>Lucknow, India</span>
              <br />
